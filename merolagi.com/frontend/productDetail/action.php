...
if (isset($_POST['makeADeal'])) {
    ...
    $insertTransaction = "INSERT INTO `deal_product`(`productId`, `customerId`, `transactionId`, `latitude`, `longitude`, `address`, `phone`, `quantity`, `createdDate`) VALUES ('" . $productId . "','" . $customerId . "','" . $transactionId . "','" . $latitude . "','" . $longitude . "','" . $address . "','" . $phoneNumber . "','" . $quantity . "','" . date("Y/m/d") . "')";

    $insertTransaction = mysqli_query($conn3, $insertTransaction);


    $getId = "SELECT * FROM `deal_product` where `transactionId`='" . $transactionId . "'";
    $getId = mysqli_query($conn3, $getId);
    $getIdData = mysqli_fetch_assoc($getId);
    $getId = "SELECT * FROM `product_info` where `productUniqueId`='" . $getIdData['productId'] . "'";
    $getId = mysqli_query($conn2, $getId);
    $getIdData = mysqli_fetch_assoc($getId);

    header("Location: /productdetail/" . $getIdData['id']."/success");
}
...
if (isset($_POST['negotiateBtn'])) {
    ...
    $insertTransaction = "INSERT INTO `product_negotiate`(`productId`, `userId`, `negotiateId`, `price`, `message`, `sentDate`) VALUES ('" . $productId . "','" . $customerId . "','" . $transactionId . "','" . $price . "','" . $comment . "','" . date("Y/m/d") . "')";

    $insertTransaction = mysqli_query($conn2, $insertTransaction);


    $getId = "SELECT * FROM `product_negotiate` where `negotiateId`='" . $transactionId . "'";
    ...
    header("Location: /productdetail/" . $getIdData['id']."/negotiateSent");
