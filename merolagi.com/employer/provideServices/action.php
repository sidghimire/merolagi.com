...
if (isset($_POST['sellBtn'])) {
    ...
    while ($a == true) {
        ...
        if (mysqli_num_rows($checkUniqueIdExistsProduct) != 0 && mysqli_num_rows($checkUniqueIdExistsService) != 0) {
            $a = true;
        } else {
            $a = false;
        }
    ...
    $sqlUploadInfo = "INSERT INTO `service_info`(`employerUniqueId`, `serviceName`, `category`, `serviceDescription`, `currency`, `price`, `workAnywhere`, `activeStatus`, `latitude`, `longitude`, `address`, `deadline`, `createdDate`, `productUniqueId`) VALUES ('" . $_SESSION['employerId'] . "','" . $serviceName . "','" . $category . "','" . $serviceDescription . "','" . $currency . "','" . $price . "','" . $workAnywhere . "','" . $activeStatus . "','" . $latitude . "','" . $longitude . "','" . $address . "','" . $deadline . "','" . date("Y/m/d") . "','" . $uniqueProductId . "')";
    ...
    for ($i = 0; $i < sizeof($featurePost); $i++) {
        ...
        $sqlFeatureUpload = "INSERT INTO `service_features`(`employerUniqueId`, `productUniqueId`, `productFeature`, `productFeatureValue`, `createdDate`) VALUES ('" . $_SESSION['employerId'] . "','" . $uniqueProductId . "','" . $feature . "','" . $value . "','" . date("Y/m/d") . "')";
    ...
    for ($i = 0; $i < sizeof($imageName); $i++) {
        ...
        $sqlImageUpload = "INSERT INTO `service_images`(`employerUniqueId`, `productUniqueId`, `imageName`, `createdDate`) VALUES ('" . $_SESSION['employerId'] . "','" . $uniqueProductId . "','" . $newFileName . "','" . date("Y/m/d") . "')";
        ...
        $sqlDirector = "SELECT id FROM `service_info` WHERE `productUniqueId`='" . $uniqueProductId . "'";
        $sqlDirector = mysqli_query($conn2, $sqlDirector);
        $sqlDirectorData = mysqli_fetch_assoc($sqlDirector);
        header("Location: /servicedetail/".$sqlDirectorData['id']);
    }
} else {
