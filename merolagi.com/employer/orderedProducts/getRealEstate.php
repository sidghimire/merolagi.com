...
<tbody>
    <?php
    $sqlAllPost = "SELECT * FROM `deal_realestate` WHERE customerId='" . $_SESSION['employerId'] . "' ORDER BY id DESC";
    $sqlAllPost = mysqli_query($conn3, $sqlAllPost);

    $i = 1;
    while ($sqlAllPostData = mysqli_fetch_assoc($sqlAllPost)) {
        $sqlProductInfo = "SELECT * FROM `realestate_info` WHERE productUniqueId='" . $sqlAllPostData['productId'] . "'";
